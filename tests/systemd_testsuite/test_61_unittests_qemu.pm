...
sub run {
    ...
    assert_script_run './run-integration-tests.sh TEST-61-UNITTESTS-QEMU --run 2>&1 | tee /tmp/testsuite.log', $timeout;
    assert_script_run 'grep "PASS: ...TEST-61-UNITTESTS-QEMU" /tmp/testsuite.log';
    script_run './run-integration-tests.sh TEST-61-UNITTESTS-QEMU --clean';
}
