...
sub run {
    ...
    assert_script_run './run-integration-tests.sh TEST-44-LOG-NAMESPACE --run 2>&1 | tee /tmp/testsuite.log', $timeout;
    assert_script_run 'grep "PASS: ...TEST-44-LOG-NAMESPACE" /tmp/testsuite.log';
    script_run './run-integration-tests.sh TEST-44-LOG-NAMESPACE --clean';
}
