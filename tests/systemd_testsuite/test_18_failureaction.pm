...
sub run {
    ...
    assert_script_run 'export NO_BUILD=1 && make -C TEST-18-FAILUREACTION run 2>&1 | tee /tmp/testsuite.log', $timeout;
    assert_script_run 'grep "TEST-18-FAILUREACTION RUN: .* \[OK\]" /tmp/testsuite.log';
}
