...
sub run {
    ...
    assert_script_run './run-integration-tests.sh TEST-63-PATH --run 2>&1 | tee /tmp/testsuite.log', $timeout;
    assert_script_run 'grep "PASS: ...TEST-63-PATH" /tmp/testsuite.log';
    script_run './run-integration-tests.sh TEST-63-PATH --clean';
}
