...
sub run {
    ...
    assert_script_run './run-integration-tests.sh TEST-64-UDEV-STORAGE --run 2>&1 | tee /tmp/testsuite.log', $timeout;
    assert_script_run 'grep "PASS: ...TEST-64-UDEV-STORAGE" /tmp/testsuite.log';
    script_run './run-integration-tests.sh TEST-64-UDEV-STORAGE --clean';
}
