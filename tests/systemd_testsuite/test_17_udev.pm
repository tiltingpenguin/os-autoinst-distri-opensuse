...
sub run {
    ...
    assert_script_run 'export NO_BUILD=1 && make -C TEST-17-UDEV run 2>&1 | tee /tmp/testsuite.log', $timeout;
    assert_script_run 'grep "TEST-17-UDEV RUN: .* \[OK\]" /tmp/testsuite.log';
}
