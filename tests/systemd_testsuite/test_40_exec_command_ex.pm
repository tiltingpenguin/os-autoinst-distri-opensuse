...
sub run {
    ...
    assert_script_run './run-integration-tests.sh TEST-40-EXEC-COMMAND-EX --run 2>&1 | tee /tmp/testsuite.log', $timeout;
    assert_script_run 'grep "PASS: ...TEST-40-EXEC-COMMAND-EX" /tmp/testsuite.log';
    script_run './run-integration-tests.sh TEST-40-EXEC-COMMAND-EX --clean';
}
