...
sub run {
    ...
    assert_script_run 'export NO_BUILD=1 && make -C TEST-40-EXEC-COMMAND-EX run 2>&1 | tee /tmp/testsuite.log', $timeout;
    assert_script_run 'grep "TEST-40-EXEC-COMMAND-EX RUN: .* \[OK\]" /tmp/testsuite.log';
    script_run 'export NO_BUILD=1 && make -C TEST-40-EXEC-COMMAND-EX clean';
}
