...
sub run {
    ...
    assert_script_run './run-integration-tests.sh TEST-19-DELEGATE --run 2>&1 | tee /tmp/testsuite.log', $timeout;
    assert_script_run 'grep "PASS: ...TEST-19-DELEGATE" /tmp/testsuite.log';
    assert_script_run './run-integration-tests.sh TEST-19-DELEGATE --clean', 120;
}
