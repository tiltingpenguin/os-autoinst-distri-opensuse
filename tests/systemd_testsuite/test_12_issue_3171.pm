...
sub run {
    ...
    assert_script_run 'export NO_BUILD=1 && make -C TEST-12-ISSUE-3171 run 2>&1 | tee /tmp/testsuite.log', $timeout;
    assert_script_run 'grep "TEST-12-ISSUE-3171 RUN: .* \[OK\]" /tmp/testsuite.log';
}
