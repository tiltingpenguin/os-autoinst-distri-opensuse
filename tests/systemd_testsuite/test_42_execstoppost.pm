...
sub run {
    ...
    assert_script_run 'export NO_BUILD=1 && make -C TEST-42-EXECSTOPPOST run 2>&1 | tee /tmp/testsuite.log', $timeout;
    assert_script_run 'grep "TEST-42-EXECSTOPPOST RUN: .* \[OK\]" /tmp/testsuite.log';
