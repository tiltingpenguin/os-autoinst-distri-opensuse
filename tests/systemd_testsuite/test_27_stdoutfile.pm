...
sub run {
    ...
    assert_script_run './run-integration-tests.sh TEST-27-STDOUTFILE --run 2>&1 | tee /tmp/testsuite.log', $timeout;
    assert_script_run 'grep "PASS: ...TEST-27-STDOUTFILE" /tmp/testsuite.log';
    script_run './run-integration-tests.sh TEST-27-STDOUTFILE --clean';
}
