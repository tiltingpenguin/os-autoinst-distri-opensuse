...
sub run {
    ...
    assert_script_run 'export NO_BUILD=1 && make -C TEST-72-SYSUPDATE run 2>&1 | tee /tmp/testsuite.log', $timeout;
    assert_script_run 'grep "TEST-72-SYSUPDATE RUN: .* \[OK\]" /tmp/testsuite.log';
    script_run 'export NO_BUILD=1 && make -C TEST-72-SYSUPDATE clean';
}
