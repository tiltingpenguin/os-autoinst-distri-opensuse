...
sub run {
    ...
    assert_script_run './run-integration-tests.sh TEST-73-LOCALE --run 2>&1 | tee /tmp/testsuite.log', $timeout;
    assert_script_run 'grep "PASS: ...TEST-73-LOCALE" /tmp/testsuite.log';
    script_run './run-integration-tests.sh TEST-73-LOCALE --clean';
}
