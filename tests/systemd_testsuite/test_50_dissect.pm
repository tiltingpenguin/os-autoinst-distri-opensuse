...
sub run {
    ...
    assert_script_run './run-integration-tests.sh TEST-50-DISSECT --run 2>&1 | tee /tmp/testsuite.log', $timeout;
    assert_script_run 'grep "PASS: ...TEST-50-DISSECT" /tmp/testsuite.log';
    script_run './run-integration-tests.sh TEST-50-DISSECT --clean';
}
