...
sub run {
    ...
    assert_script_run 'export NO_BUILD=1 && make -C TEST-77-OPENFILE run 2>&1 | tee /tmp/testsuite.log', $timeout;
    assert_script_run 'grep "TEST-77-OPENFILE RUN: .* \[OK\]" /tmp/testsuite.log';
    script_run 'export NO_BUILD=1 && make -C TEST-77-OPENFILE clean';
}
