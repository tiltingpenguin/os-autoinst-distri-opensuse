...
sub run {
    ...
    assert_script_run 'export NO_BUILD=1 && make -C TEST-01-BASIC run 2>&1 | tee /tmp/testsuite.log', $timeout;
    assert_script_run 'grep "TEST-01-BASIC RUN: .* \[OK\]" /tmp/testsuite.log';
}
