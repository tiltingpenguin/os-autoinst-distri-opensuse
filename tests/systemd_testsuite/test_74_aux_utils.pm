...
sub run {
    ...
    assert_script_run './run-integration-tests.sh TEST-74-AUX-UTILS --run 2>&1 | tee /tmp/testsuite.log', $timeout;
    assert_script_run 'grep "PASS: ...TEST-74-AUX-UTILS" /tmp/testsuite.log';
    script_run './run-integration-tests.sh TEST-74-AUX-UTILS --clean';
}
