...
sub run {
    ...
    assert_script_run 'export NO_BUILD=1 && make -C TEST-32-OOMPOLICY run 2>&1 | tee /tmp/testsuite.log', $timeout;
    assert_script_run 'grep "TEST-32-OOMPOLICY RUN: .* \[OK\]" /tmp/testsuite.log';
    script_run 'export NO_BUILD=1 && make -C TEST-32-OOMPOLICY clean';
}
