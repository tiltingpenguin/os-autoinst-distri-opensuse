...
sub run {
    ...
    assert_script_run 'export NO_BUILD=1 && make -C TEST-03-JOBS run 2>&1 | tee /tmp/testsuite.log', $timeout;
    assert_script_run 'grep "TEST-03-JOBS RUN: .* \[OK\]" /tmp/testsuite.log';
}
