...
sub run {
    ...
    assert_script_run './run-integration-tests.sh TEST-28-PERCENTJ-WANTEDBY --run 2>&1 | tee /tmp/testsuite.log', $timeout;
    assert_script_run 'grep "PASS: ...TEST-28-PERCENTJ-WANTEDBY" /tmp/testsuite.log';
    script_run './run-integration-tests.sh TEST-28-PERCENTJ-WANTEDBY --clean';
}
