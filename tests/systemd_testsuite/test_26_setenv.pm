...
sub run {
    ...
    assert_script_run './run-integration-tests.sh TEST-26-SETENV --run 2>&1 | tee /tmp/testsuite.log', $timeout;
    assert_script_run 'grep "PASS: ...TEST-26-SETENV" /tmp/testsuite.log';
    script_run './run-integration-tests.sh TEST-26-SETENV --clean';
}
